...
- When adding a new plugin, create its directory under `plugins/<domain>/` with the required files. **Do not modify** `.agents/plugins/marketplace.json` or `.Codex-plugin/marketplace.json` — these files are auto-generated by the `sync-registry.yml` workflow after every merge to `main`. Any PR that touches them will be rejected by CI.
- When modifying any file inside a skill directory, you must also:
  ...
  2. Bump the version in both `.Codex-plugin/plugin.json` and `.codex-plugin/plugin.json` for the parent plugin. Both files must stay in sync. CI enforces this on every PR.
...
- New plugins have their directory created under `plugins/<domain>/` — do not touch `.agents/plugins/marketplace.json` or `.Codex-plugin/marketplace.json` (auto-generated by CI).
